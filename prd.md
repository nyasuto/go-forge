...
- [x] Tier 2: `-n` 最大引数数指定、`-P` 並列実行数指定。テスト追加・通過。テスト結果を更新。
- [x] Tier 3: `-0` null区切り対応、`--dry-run` 実行せずコマンドを表示。テスト追加・通過。テスト結果を更新。
...
- [x] Tier 2: unified diff format (`-u`) 出力。テスト追加・通過。テスト結果を更新。
- [x] Tier 3: カラー出力（ターミナル検出）、`--word` 単語単位diff。テスト追加・通過。テスト結果を更新。
...
- [x] Tier 3: `select(条件)` フィルタ、`keys`、`values`。テスト追加・通過。テスト結果を更新。
